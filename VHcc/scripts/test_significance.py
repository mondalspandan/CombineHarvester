...
#           backgroundComp("VH(H#rightarrowc#bar{c})"+(args.mu),["ZH_hcc","WH_hcc","ggZH_hcc"],2),
...
for i in range(1,splusbhist.GetNbinsX()+1):
  ...
  print "signal prefit events in bin ", i, "=  ",sighist_prefit.GetBinContent(i)," bkghist= ", bkghist.GetBinContent(i), "  ------> S/sqrt(S+b): ", sighist_prefit.GetBinContent(i)/sqrt(sighist_prefit.GetBinContent(i)+bkghist.GetBinContent(i))

    ...
    for CHN in ['Wen','Wmn','Zee','Zmm','Znn']:
        for i in range(0,len(CHN_DICT_SR[CHN])):
              ...
              os.system(('./scripts/postFitPlot_vhcc.py' \
                  ' --file=shapes_10Feb_stable.root --ratio --extra_pad=0.53 --file_dir=%(OUTNAME)s ' \
                             ' --ratio_range 0.4,1.6 --empty_bin_error --channel=%(CHN)s --blind --x_blind_min 0.5 --x_blind_max 1.0 --x_title BDT' \

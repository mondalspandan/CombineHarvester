...
import CombineHarvester.VHcc.systematics_vhcc_test as systs
import ROOT as R
import glob
import numpy as np
import os
...
if args.mjj:
    ...
    cats = {
      ...
      'Zmm' : [
        ...
        (9,'Zcc_high_Zmm'), (10,'Zcc_low_Zmm')
        #(5,'Zcc_high_Zmm'), (6,'Zcc_low_Zmm'),(7,'ttbar_high_Zmm'), (8,'ttbar_low_Zmm'),
        ],
      'Wen' : [
        (1, 'SR_Wenu'), (3,'Wlf_Wenu'), (5,'Whf_Wenu'), (9,'Wcc_Wenu')
        ],
      'Wmn' : [
        (1, 'SR_Wmunu'), (3,'Wlf_Wmunu'), (5,'Whf_Wmunu'), (9,'Wcc_Wmunu')
        ],
      'Znn' : [
        (1, 'SR_Znn'), (3,'Vlf_Znn'), (5,'Vhf_Znn'), (9,'Vcc_Znn')
        ]
      }
...
if 'Znn' in chns:
  ...
  if args.mjj:
    ...
    writer.WriteCards("Znn",cb.cp().bin_id([3,5,9]).channel(['Wmn','Wen']))
      #Luca writer.WriteCards("Znn_CRonly",cb.cp().bin_id([3,4,5,6,7,8]).channel(['Znn','Wmn','Wen']))
  else:
    writer.WriteCards("Znn",cb.cp().channel(['Znn']))
    writer.WriteCards("Znn",cb.cp().bin_id([5,6,8]).channel(['Wmn','Wen']))
    writer.WriteCards("Znn_CRonly",cb.cp().bin_id([3]).channel(['Znn']))

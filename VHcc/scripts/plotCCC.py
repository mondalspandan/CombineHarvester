...
axis = ROOT.TH2F('axis', '',1,-75,125,7,0,7)
...
txt_dict = {
  ...
  'r_zerolep': '#splitline{0 lept.}{#mu=%.0f#pm%.0f}'%(js['r_zerolep']['val'],(js['r_zerolep']['ErrHi']+js['r_zerolep']['ErrLo'])/2.),
  'r_onelep': '#splitline{1 lept.}{#mu=%.0f#pm%.0f}'%(js['r_onelep']['val'],(js['r_onelep']['ErrHi']+js['r_onelep']['ErrLo'])/2.),
  'r_twolep': '#splitline{2 lept.}{#mu=%.0f#pm%.0f}'%(js['r_twolep']['val'],(js['r_twolep']['ErrHi']+js['r_twolep']['ErrLo'])/2.)
}
...
latex.DrawLatex(-69,5.6,"pp#rightarrow VH(H#rightarrow c#bar{c})")

    ...
    for CHN in ['Zee','Zmm','Wen','Wmn','Znn']:
        ...
        for i in range(0,len(CHN_DICT_CRHF[CHN])):
            ...
            os.system(('./scripts/postFitPlot_vhcc_alt.py' \
                       ' --file=shapes_UNBL_Nov21_Ak15Pt300_FullRun2_VHcc.root --ratio --extra_pad=0.53 --file_dir=%(OUTNAME)s --outdir="Nov21_unbl_plots/2016/" --no_signal ' \
                       ' --ratio_range 0.4,1.6 --empty_bin_error --channel=%(CHN)s --cr ' \
                       ' --outname %(OUTNAME)s --mode %(MODE)s --x_title="CvsB_{min}"  --y_title "Events" --lumi="36.3 fb^{-1} (13 TeV)"'\
    ...
    #for CHN in ['Znn']:
        for i in range(0,len(CHN_DICT_CRLF[CHN])):
            ...
            os.system(('./scripts/postFitPlot_vhcc_alt.py' \
                       ' --file=shapes_UNBL_Nov21_Ak15Pt300_FullRun2_VHcc.root --ratio --extra_pad=0.53 --file_dir=%(OUTNAME)s --outdir="Nov21_unbl_plots/2016/" --no_signal ' \
                       ' --ratio_range 0.4,1.6 --empty_bin_error --channel=%(CHN)s --cr ' \
                       ' --outname %(OUTNAME)s --mode %(MODE)s --x_title="CvsL_{min}"  --y_title "Events" --lumi="36.3 fb^{-1} (13 TeV)"'\
                       ' --x_axis_min %(XLOW)f --x_axis_max %(XHIGH)f --custom_x_range '\
                       ' --channel_label "%(LABEL)s" --extralabel "%(EXTRALABEL)s"' % vars()))
